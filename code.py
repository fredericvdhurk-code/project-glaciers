...
def load_data(norm_period):
    """Load data for the specified norm period"""
...
def run_regression_analysis(mass_balance_df, glacier_mappings, temp_cols, prec_cols, analysis_name, norm_period):
    """Run regression analysis for a specific configuration"""
    print(f"\n{'='*80}")
...
def run_all_analyses():
    """Run all regression analyses for both norm periods"""
    for norm_period in ["1961-1990", "1991-2020"]:
        print(f"\n{'='*80}")
        print(f"RUNNING ANALYSES FOR {norm_period} NORMS")
        print('='*80)

        mass_balance_df, glacier_mappings = load_data(norm_period)

        # 1. Monthly deviations analysis
        ...
        run_regression_analysis(mass_balance_df, glacier_mappings, monthly_temp_cols, monthly_prec_cols,
                               "MONTHLY DEVIATIONS", norm_period)

        # 2. Optimal seasonal deviations analysis
        opt_temp_cols = ['opt_season_td']
        opt_prec_cols = ['opt_season_pd']
        run_regression_analysis(mass_balance_df, glacier_mappings, opt_temp_cols, opt_prec_cols,
                               "OPTIMAL SEASONAL DEVIATIONS", norm_period)

        # 3. Summer/Winter seasonal analysis
        season_temp_cols = ['summer_td']
        season_prec_cols = ['winter_pd']
        run_regression_analysis(mass_balance_df, glacier_mappings, season_temp_cols, season_prec_cols,
                               "SUMMER/WINTER SEASONAL DEVIATIONS", norm_period)
